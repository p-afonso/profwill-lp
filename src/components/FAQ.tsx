...
export default function FAQ() {
  return (
    <section className="py-20 bg-[#011E5A]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {faqs.map((faq, index) => (
            <div key={index} className="bg-white rounded-xl p-6 shadow-lg">
              <div className="flex items-start gap-4 mb-3">
                <div className="bg-[#6C39C1] rounded-full p-3 flex-shrink-0">
                  <faq.icon className="w-6 h-6 text-white" />
                </div>
                <h3 className="text-xl font-bold text-[#6C39C1] pt-2">{faq.question}</h3>
              </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {quickFeatures.map((feature, index) => (
            <div
              key={index}
              className="bg-gradient-to-br from-blue-100 to-blue-50 rounded-xl p-6 text-center hover:scale-105 transition-transform"
            >
              <div className="bg-[#011E5A] rounded-full w-14 h-14 flex items-center justify-center mx-auto mb-4">
                <feature.icon className="w-7 h-7 text-[#F97430]" />
              </div>
              <h4 className="text-xl font-bold text-[#011E5A] mb-2">{feature.title}</h4>
